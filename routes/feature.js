...
var upload = multer({storage:storage, fileFilter:fileFilter,
     ...
     router.get('/', async(req, res)=>{
        const news1 = await Feature.find({category: 'news'}).sort({date:-1}).skip(4).limit(1).populate(' user') 
        const news2 = await Feature.find({category: 'news'}).sort({date:-1}).skip(3).limit(1).populate('user')
        const news3 = await Feature.find({category: 'news'}).sort({date:-1}).skip(2).limit(1).populate(' user')
        const pols1 = await Feature.find({category: 'politics'}).sort({date:-1}).skip(5).limit(1).populate(' user')
        const pols2 = await Feature.find({category: 'politics'}).sort({date:-1}).skip(6).limit(1).populate(' user')
        const pols3 = await Feature.find({category: 'politics'}).sort({date:-1}).skip(4).limit(1).populate(' user')
        const edu1 = await Feature.find({category: 'education'}).sort({date:-1}).limit(1).skip(4).populate(' user')
        const edu2 = await Feature.find({category: 'education'}).sort({date:-1}).limit(1).skip(3).populate(' user')
        const edu3 = await Feature.find({category: 'education'}).sort({date:-1}).limit(1).skip(2).populate(' user')
        const ent1 = await Feature.find({category: 'entertainment'}).sort({date:-1}).skip(4).limit(1).populate(' user')
        const ent2 = await Feature.find({category: 'entertainment'}).sort({date:-1}).skip(3).limit(1).populate(' user')
